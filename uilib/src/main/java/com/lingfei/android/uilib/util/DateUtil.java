...
public final class DateUtil implements Serializable{
				...
				private static final long THIRD_DAY = DAY * 3;// 3天的秒数
				...
				public static final SimpleDateFormat FORMAT_YYYYMD = new SimpleDateFormat("yyyy-M-d");
				...
				/**
					...
					* @return
					*/
				public static String getTimePassedDay(Date date, int gap,
				                                      SimpleDateFormat format){
								Calendar cal = Calendar.getInstance();
								cal.setTime(date);
				...
				public static Date parseString(String strDate, SimpleDateFormat format){
								try{
												...
												LoggerUtil.e("DateUtil", e.toString());
								}
				...
				/**
					...
					* @return
					*/
				public static String getLeftTime(long timestamp){
								long currentTime = System.currentTimeMillis();
				...
				/**
					...
					* @return
					*/
				public static boolean isAfterToday(long mill){
								return isBefore(getCurrentTime(), getDateTime(mill), "yyyy-MM-dd HH:mm:ss");
				}
				...
				public static List<String> getDays(String startDate, String endDate){
								...
								if(!dates.isEmpty()){
												for(Date date : dates){
																dayList.add(convertDate(date.getTime(), "yyyyMMdd"));
												}
								}
				...
				public static List<Date> getDates(String startDate, String endDate){
								...
								try{
												...
												p_end = Calendar.getInstance();
												p_end.setTime(dateEnd);
								}
								catch(ParseException e){
												LoggerUtil.e("DateUtil", e.toString());
								}

								if(null != temp && null != p_end){
												while(temp.before(p_end) || temp.getTime().equals(p_end.getTime())){
				...
				/**
					* 比较传入的时间和当前时间的差值是否小于24小时
					*
					* @param timestamp
					* @return
					*/
				public static int getLE24Hours(long timestamp){
								long currentTime = System.currentTimeMillis();
								long timeGap = THIRD_DAY - (currentTime - timestamp) / 1000;// 与现在时间相差秒数
								int timeStr = 0;
								if(timeGap > HOUR && timeGap < DAY){// 小于24小时
												timeStr = (int) (timeGap / HOUR);
								}
								else if(timeGap > 0 && timeGap < HOUR){
												timeStr = 1;
								}
								else{ // 大于24小时
												timeStr = 25;
